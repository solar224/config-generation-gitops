# Operation Guide — Local Deployment (Kind)

This guide covers bootstrapping and operating the Config Generation platform on a local [Kind](https://kind.sigs.k8s.io/) cluster.
...
**Ports 80 and 443 on the host machine must be reachable from the internet** (the cluster must be able to reach the Cloudflare API for DNS-01 validation; no inbound port requirement for DNS-01 itself).
